...
width = 10.
height = 1.
camera_position = np.array([12.47316, 41.89247, 0.004])
...
def capture_screen(x, y, width, height):
    """Cattura lo schermo da (x, y) a (x+width, y+height) e restituisce un'immagine PIL."""
    pixels = glReadPixels(x, y, width, height, GL_RGB, GL_UNSIGNED_BYTE)
    image = Image.frombytes("RGB", (width, height), pixels)
    # Poiché glReadPixels legge dal basso verso l'alto, devi invertire l'immagine.
...
def transform_and_filter_coordinates(coords, projection_matrix, modelview_matrix, viewport):
    transformed_coords = []
    # Va fatta una trasformazione di coordinate perché x, y sono espresse in lat, lon.
    # Si può evitare di farla se si crea in origine una lista con tutti i valori corretti
    # nella funzione get_tiber()
    for (x, y) in coords:
        screen_x, screen_y, _ = gluProject(55*(x-camera_position[0]), 74*(y-camera_position[1]), 0, 
...
def display():
    ...
    K=0.3
    h_angle = math.radians(90)
    v_angle = math.radians(45)
    # Set up camera position and orientation
    gluLookAt(-K*math.sin(h_angle)*math.cos(v_angle), -K*math.cos(h_angle)*math.cos(v_angle), K*math.sin(v_angle),  # Eye position (camera position)
              0, 0, 0,  # Look-at point (camera is looking at this point)
              0, 0, 1)  # Up vector (camera's up direction)
...
def keyboard_callback(key, x, y):
    if key == b'\r':  # Questo è il codice per il tasto 'Enter'
        ...
        # Export a screenshot of the view
        screen_image = capture_screen(0, 0, viewport[2], viewport[3])
...
def main():
    ...
    texture_id = load_texture("printmaps.png")
